...
When equal variances are not assumed (Welch’s *t*-test), the standard error of the difference is calculated using @eq-indttest-unequalvar, without pooling.
...
The calculate p-value is `{r} round(p, 4)`, which is greater than the significance threshold (0.05). Therefore, we fail to reject the null hypothesis that there is no difference in coffee productivity between farms that use or do not use pesticides. We conclude that using pesticides do not lead to an increase in coffee productivity in our example.
:::
::::
