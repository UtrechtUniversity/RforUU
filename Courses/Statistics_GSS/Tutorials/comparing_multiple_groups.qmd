...
$$
SS_{factor} = \sum_{i=1}^k\sum_{j=1}^{n_i} (\bar{y}_{i} - \bar{y})^2
$$ {#eq-SSfactor}
...
| Residuals | $SS_{res}$ | $(n-1)k$ | $MS_{res}=\frac{SS_{res}}{(n-1)k}$ |  |

: Structure of a one-way ANOVA table. *n* is the number of observations per group, while *k* is the number of groups being compared. {#tbl-anovatable}
