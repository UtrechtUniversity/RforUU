...
To work on this tutorial, you will need to load the *tidyverse*, *lmerTest*, and *ggeffects* packages.
...
2.  Use `lmerTest::lmer()` to fit a mixed-effects model (random intercepts) using mean rooting depth (`mrd`) as the response variable, and time (`dafs`), plant order of arrival (`Treatment`), and their interaction as fixed effects. Use the rhizobox ID number (`Rz`) as a random effect. Store the results in an object called `model2`.
...
This pattern is reflected in the coefficients shown in the summary table. In the reference treatment (F-first, which is used as the reference level because it comes first alphabetically), the slope of the relationship between mean rooting depth and time is negative (−0.259). This means that, in this treatment, the mean rooting depth decreases over time at a rate of 0.259 cm per day. In the scenario where grasses were sown first (G-first), the slope increases by 0.05 relative to the F-first treatment. As a result, the slope in the G-first treatment becomes −0.259 + 0.05 = −0.209. In other words, the decline in mean rooting depth over time is less pronounced when grasses are sown first. The summary table also indicates that, on average, plant communities in which legumes were sown first tend to have deeper roots than communities in which forbs were sown first.
